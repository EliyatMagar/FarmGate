...
import type { Order, OrderFilters, CreateOrderData, UpdateOrderStatusData, UpdatePaymentStatusData } from '../../../types/order';
...
export const fetchFarmerOrders = createAsyncThunk(
  ...
  async (filters: OrderFilters = {}, { rejectWithValue }) => {
    try {
      const response = await orderAPI.getFarmerOrders(filters);
      return response;
    } catch (error: any) {
...
export const updatePaymentStatus = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: UpdatePaymentStatusData }, { rejectWithValue }) => {
    try {
      const response = await orderAPI.updatePaymentStatus(id, data);
      return response;
    } catch (error: any) {
